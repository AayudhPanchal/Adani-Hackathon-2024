  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
        <WeatherGauge
          value={mockData.weather.temperature}
          max={50}
          title="Temperature"
          icon={Sun}
          color="#ef4444"
        />
        <WeatherGauge
          value={mockData.weather.windSpeed}
          max={30}
          title="Wind Speed"
          icon={Wind}
          color="#3b82f6"
        ...
        />
        <WeatherGauge
          value={mockData.weather.cloudCover}
          max={100}
          title="Cloud Cover"
          icon={Battery}
          color="#eab308"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Object.entries(weatherImpactData).map(([source, data]) => (
          <div
            key={source}
            className="bg-slate-800 p-6 rounded-lg shadow-lg hover:shadow-xl transition duration-300 ease-in-out"
          >
            <h3 className="text-lg font-semibold capitalize mb-4 text-white">
              {source} Generation
            </h3>
...
export const GenerateEnergy = () => {
  ...
  const fetchPredictions = async (start, end) => {
    setIsLoading(true);
    try {
      const response = await fetch(`${process.env.ML_BACKEND_URL}/predict/power`, {
        method: "POST",
        headers: {
      ...
      <div className="bg-slate-800 p-6 rounded-lg">
        ...
        {predictions.date_range && (
          <div className="mt-4 text-sm text-slate-400 text-center">
            Predictions for period: {predictions.date_range.start} to {predictions.date_range.end}
          </div>
        )}
      </div>
    );
